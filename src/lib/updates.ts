...
function evaluateReleases(releases: GithubRelease[], manual: boolean): {
  ...
  for (const rel of validReleases) {
    ...
    if (compareVersions(rawVer, __APP_VERSION__) > 0) {
      ...
      toast.info(`Nueva versión para PC: ${rel.name || `v${rawVer}`}`, {
        description: rel.body ? rel.body.slice(0, 120) : 'Mejoras y correcciones disponibles.',
        duration: 15000,
        action: exeAsset ? {
          label: 'Descargar .EXE',
          onClick: () => window.open(exeAsset.browser_download_url, '_blank'),
        } : (rel.html_url ? {
          label: 'Ver en GitHub',
  ...
  if (manual) {
    toast.success(`El sistema está actualizado. Versión actual: v${__APP_VERSION__}`);
  }
